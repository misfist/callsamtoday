...
/***************************************************************************/	
	...
	function themify_background_image($data=array()){
		...
		$output .= '<div class="themify_field_row background_image">
						...
						<input type="checkbox" class="noBgImage" name="styling-'.$data['category'].'-'.$data['title'].'-background_image-value-none" '.$none_checked.' /> ' . __('No BG image', 'themify') . '<br /> 
		...
		if($data['value']['transparent']){
			$output = '<div class="themify_field_row themify_field-background_color">
						...
						<div class="themify_field-color">
							<span class="colorSelect" style=""><span></span></span> <input type="text" disabled name="styling-'.$data['category'].'-'.$data['title'].'-background_color-value-value" class="colorSelectInput width4 opacity-7" /> 
	...
	function themify_font_family($data=array()){
		...
		if ( sizeof( $themify_gfonts ) > 0 ) {
			...
			foreach ( $themify_gfonts as $font ) {
				$selected = ( $font == $data['value']['value'] ) ? ' selected="selected"' : '';
	...
	function themify_color($data=array()){
		...
		return '<div class="themify_field_row">
					...
					<div class="themify_field-color">
						...
						<input type="text" class="colorSelectInput width4" value="'.$data['value']['value'].'" name="styling-'.$data['category'].'-'.$data['title'].'-color-value-value" />
						<input type="button" class="button clearColor" value="' . __('Clear', 'themify') . '">
					</div>
				</div>';
	}
...
function themify_gallery_plugins($data=array()){
	...
	$out = '<p>
				...
				foreach($display_options as $option => $value){
					$out .= '<option value="'.$value.'" '.selected($value? $value: 'prettyPhoto', $data['setting-gallery_lightbox'], false).'>'.$option.'</option>';
				}
	$out .= '	</select>
			</p>';
	$out .= '<p>
				<span class="pushlabel"><label for="setting-lightbox_content_images">
					<input type="checkbox" id="setting-lightbox_content_images" name="setting-lightbox_content_images" '. checked($data['setting-lightbox_content_images'], 'on', false) .'/> ' . __('Apply lightbox to image links (ie. links to jpg, png, and gif will open in lightbox)', 'themify') . '</label>
				</span>
			</p>';
	return $out;
}
...
function themify_open_graph_options($data = array()){
	...
	$out .= '<p>
				<span><label for="'.$pre.'disable">
					<input type="checkbox" id="'.$pre.'disable" name="'.$pre.'disable" '. checked($data[$pre.'disable'], 'on', false) .'/> ' . __('Remove Open Graph Tags', 'themify') . '</label>
				</span>
			</p>';
	return $out;
}
...
function themify_post_meta_options($pmkey, $data, $metas = array(), $states = array(), $group_label = false) {
	...
	$out = '<div class="themify_field_row dropdownbutton-group"><span class="label">' . $group_label . '</span>';
			...
			foreach($metas as $meta => $name) {
				...
				$out .= '<input type="hidden" value="'.$data[$metakey].'" class="'.$meta_class.'" id="'.$metakey.'" name="'.$metakey.'" />';
			}
		...
		$out .= '<p>
					...
					<select name="'.$key.'by">';
						foreach($orderby_options as $option => $value){
							$out .= '<option value="'.$value.'" '.selected($value? $value: 'date', $data[$key.'by'], false).'>'.$option.'</option>';
							}
		$out .= '	</select>
				</p>
				<p>
					...
					<select name="'.$key.'">';
						foreach($order_options as $option => $value){
							$out .= '<option value="'.$value.'" '.selected($value? $value: 'DESC', $data[$key], false).'>'.$option.'</option>';
					}
		$out .= '	</select>
				</p>';
...
if(!function_exists('themify_exclude_search_form')){
	...
	function themify_exclude_search_form(){
		...
		return '<p><label for="setting-exclude_search_form"><input type="checkbox" id="setting-exclude_search_form" name="setting-exclude_search_form" '.$pages_checked.'/> ' . __('Check here to exclude search form', 'themify') . '</label></p>';	
	}
}
